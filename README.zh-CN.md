...
以下按当前真实接入面记录。只有已经能作为用户功能使用的项目标记为完成；部分接入和未接入项目均保持未完成。最近核对时间：2026-06-23。
...
- [ ] 在应用内提供更清晰的冲突、同步失败和多步骤恢复指引。

### GitHub 项目和协作查看

- [x] GitHub 仓库元信息、star、fork、release 和默认分支状态。
- [x] issue 和 pull request 筛选列表、路由持久化筛选状态和详情侧栏。
- [x] 在 issue 和 pull request 详情中渲染 discussion Markdown 时间线。
- [x] Actions 运行详情、job 日志和 workflow 节点图。
- [x] GitHub Projects 元数据读取在缺少 `read:project` 权限时平滑降级。
- [ ] 仓库和个人维度的完整 GitHub Projects 看板查看。
- [ ] 通知收件箱，支持按仓库、issue、pull request 和 review 过滤。
- [ ] 最近推送活动、release 状态和更完整的失败 workflow 查看。
...
- [ ] 个人首页，展示分配给自己的工作、最近处理的仓库和保存的工作区。
- [ ] 关注仓库和通知偏好。
- [ ] 收藏仓库和常用工作区的本地组织。
...
发布打包由 GitHub Actions release workflow 驱动。根 `package.json` 可以使用 `0.1.0-alpha.2` 这类预发布版本名；Tauri 的 `src-tauri/tauri.conf.json` 保持 Tauri bundler 期望的数字安装包版本。

### `v0.1.0-alpha.2` 发布说明

这次预发布整理了 `v0.1.0-alpha.1` 之后、2026-06-21 到 2026-06-23 已落地的 GitHub 工作流能力：
