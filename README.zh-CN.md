...
以下按当前真实接入面记录。只有已经能作为用户功能使用的项目标记为完成；部分接入和未接入项目均保持未完成。最近核对时间：2026-06-22。
...
- [x] GitHub 仓库元信息、star、fork、topic、默认分支状态和仓库设置。
- [x] Issue 浏览、筛选、详情时间线、模板辅助创建和仓库 project 字段。
- [x] Pull Request 浏览、筛选、详情时间线、checks、创建流程和合并操作。
- [x] 基于已关联 issue 和 pull request 的仓库 Projects 看板查看。
- [x] Actions run 列表、run 详情、job 图、job 日志和 artifact 预览。
- [x] 首页 GitHub 时间线，覆盖最近 issue、pull request、workflow run、push 和同步事件。
- [ ] GitHub Discussions 浏览和创建。
- [ ] workflow run 之外的 release 列表 / 状态管理。
...
- [x] 账号级仓库列表、GitHub 仓库创建和克隆到工作区流程。
- [ ] 关注仓库和通知偏好。
- [ ] 收藏仓库和常用工作区的本地组织。
