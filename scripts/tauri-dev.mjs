...
const startPort = parsePort(process.env.LILIA_GITHUB_DEV_PORT);
...
const env = {
  ...process.env,
  LILIA_GITHUB_DEV_PORT: String(port),
  LILIA_GITHUB_DEV_STRICT_PORT: "1",
};

if (process.env.LILIA_GITHUB_DEV_DRY_RUN === "1") {
  ...
  console.log(
    JSON.stringify({
      ...
      devUrl,
      env: {
        LILIA_GITHUB_DEV_PORT: env.LILIA_GITHUB_DEV_PORT,
        LILIA_GITHUB_DEV_STRICT_PORT: env.LILIA_GITHUB_DEV_STRICT_PORT,
      },
    }),
  );
  process.exit(0);
...
console.log(`[lilia-github] Starting Tauri dev server at ${devUrl}`);
...
child.on("error", (error) => {
  console.error(`[lilia-github] Failed to start Tauri dev: ${error.message}`);
  process.exitCode = 1;
});
